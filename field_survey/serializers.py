...
class FieldSurveyTableSerializer(serializers.ModelSerializer):
    ...
    site_name = serializers.CharField(source='site_id.general_location_name', 
                                      read_only=True)

    class Meta:
        model = FieldSurvey
        fields = ['survey_global_id', 'project_ids', 'supervisor', 'username',
                  'recorder_fname', 'recorder_lname',
                  'site_id', 'site_name',
                  'env_measure_mode',
                  'survey_complete'
                  'gps_alt', 'lat', 'lon', 'srid', 'geom',
...
class FilterSampleTableSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['field_sample_barcode', 'filter_sample_label', 'survey_datetime', 'is_extracted',
                  ...
                  'sample_global_id', 'survey_global_id', 'field_sample',
                  'created_by', 'created_datetime', 'modified_datetime']
    # Since fund, system, watershed, and created_by reference different tables and we
    # want to show 'label' rather than some unintelligable field (like pk 1), have to add
    # slug to tell it to print the desired field from the other table
    ...
    survey_datetime = serializers.ReadOnlyField(source='field_sample.survey_global_id.created_datetime')
    is_extracted = serializers.ReadOnlyField(source='field_sample.field_sample_barcode.is_extracted')
    project_ids = serializers.ReadOnlyField(source='field_sample.survey_global_id.project_ids.project_label')
    supervisor = serializers.ReadOnlyField(source='field_sample.survey_global_id.supervisor.email')
    username = serializers.ReadOnlyField(source='field_sample.survey_global_id.username.email')
    site_id = serializers.ReadOnlyField(source='field_sample.survey_global_id.site_id.site_id')
    site_name = serializers.ReadOnlyField(source='field_sample.survey_global_id.site_name')
    survey_complete = serializers.ReadOnlyField(source='field_sample.survey_global_id.survey_complete')
    qa_editor = serializers.ReadOnlyField(source='field_sample.survey_global_id.qa_editor.email')
    qa_datetime = serializers.ReadOnlyField(source='field_sample.survey_global_id.qa_datetime')
    gps_alt = serializers.ReadOnlyField(source='field_sample.survey_global_id.gps_alt')
    gps_horacc = serializers.ReadOnlyField(source='field_sample.survey_global_id.gps_horacc')
    gps_vertacc = serializers.ReadOnlyField(source='field_sample.survey_global_id.gps_vertacc')
    sample_global_id = serializers.ReadOnlyField(source='field_sample.sample_global_id')
    survey_global_id = serializers.ReadOnlyField(source='field_sample.survey_global_id.pk')
...
class GeoFieldSurveySerializer(GeoFeatureModelSerializer):
    ...
    survey_datetime = serializers.DateTimeField(source='created_datetime', read_only=True)
...
class EnvMeasureSerializer(serializers.ModelSerializer):
    env_global_id = serializers.CharField(read_only=False, max_length=255, validators=[UniqueValidator(queryset=EnvMeasure.objects.all())])
    ...
    class Meta:
        ...
        fields = ['env_global_id', 'survey_global_id', 'env_measure_datetime', 'env_measure_depth', 'env_instrument', 'env_ctd_filename',
                  ...
                  'env_measure_type', 'env_flow_rate', 'env_water_temp', 'env_salinity', 'env_ph_scale', 'env_par1',
...
class FieldSampleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FieldSample
        fields = ['survey_global_id', 'sample_global_id', 'sample_material', 'is_extracted',
...
class EnvMeasureNestedSerializer(serializers.ModelSerializer):
...
class FieldSurveyEnvsNestedSerializer(GeoFeatureModelSerializer, EagerLoadingMixin):
    ...
    env_measurements = EnvMeasureNestedSerializer(many=True, read_only=True)
    created_by = serializers.SlugRelatedField(many=False, read_only=True, slug_field='email')
    project_ids = serializers.SlugRelatedField(many=True, read_only=True, allow_null=True, slug_field='project_code')
    site_id = serializers.SlugRelatedField(many=False, read_only=True, allow_null=True, slug_field='site_id')
    username = serializers.SlugRelatedField(many=False, read_only=True, slug_field='email')
