...
class FastqFile(DateTimeUserMixin):
    ...
    run_result = models.ForeignKey(RunResult, null=True, blank=True, on_delete=models.SET_NULL, related_name='run_results')
    ...
    def save(self, *args, **kwargs):
        # Build a slug even if there is no RunResult yet
        runid_part = slugify(self.run_result.run_id) if getattr(self, 'run_result_id', None) else str(self.uuid)
        fastq_part = slugify(self.fastq_filename)
        # If fastq filename is empty for any reason, fall back to UUID
        if not fastq_part:
            fastq_part = str(self.uuid)
        self.fastq_slug = f'{runid_part}_{fastq_part}'
