...
from .models import PrimerPair, IndexPair, IndexRemovalMethod, \
    ...
    RunPrep, RunResult, FastqFile, AmplificationMethod, FieldSample
...
from .renderers import ENAXMLRenderer
...
class MixsWaterFilterView(LoginRequiredMixin, 
    ...
    # permissions - https://stackoverflow.com/questions/9469590/check-permission-inside-a-template-in-django
    # View site filter view with REST serializer and django-tables2
    # Query based on FieldSample to extract data from EnvMeasure, FieldSurvey, FilterSample, SampleBarcode and SampleType
    model = FieldSample
    table_class = MixsWaterTable
    filterset_class = wetlab_filters.MixsWaterFilter
    template_name = 'home/django-material-dashboard/model-filter-list.html'
    permission_required = ('field_survey.view_fieldsample', 'wet_lab.view_fastqfile', )
    ...
    filter_backends = [filters.DjangoFilterBackend]
    exclude_columns = ()
    # Add XML to export formats
    export_formats = list(settings.EXPORT_FORMATS) + ['ena_xml']
    # Specify custom renderer for XML
    renderer_classes = {
    ...
    def get_queryset(self):
        # Query FieldSample filtered by water sample type
        # This gives access to: FieldSample, survey_global_id (FieldSurvey), 
        # field_sample_barcode (SampleBarcode), sample_type (SampleType),
        # filter_sample (FilterSample), and env_measurements through survey
        queryset = FieldSample.objects.filter(sample_type__sample_type_code='ws').select_related(
            'survey_global_id',
            'field_sample_barcode',
            'sample_type',
            'filter_sample'
        ).prefetch_related(
            'survey_global_id__env_measurements',
            'survey_global_id__site_id',
            'survey_global_id__project_ids'
        )
        return queryset
...
class MixsSedimentFilterView(LoginRequiredMixin, PermissionRequiredMixin, CharSerializerExportMixin, SingleTableMixin, FilterView):
    # permissions - https://stackoverflow.com/questions/9469590/check-permission-inside-a-template-in-django
    # View site filter view with REST serializer and django-tables2
    # Query based on FieldSample to extract data from EnvMeasure, FieldSurvey, SubCoreSample, SampleBarcode and SampleType
    model = FieldSample
    table_class = MixsSedimentTable
    filterset_class = wetlab_filters.MixsSedimentFilter
    template_name = 'home/django-material-dashboard/model-filter-list.html'
    permission_required = ('field_survey.view_fieldsample', 'wet_lab.view_fastqfile', )
    ...
    def get_queryset(self):
        # Query FieldSample filtered by sediment sample type
        # This gives access to: FieldSample, survey_global_id (FieldSurvey), 
        # field_sample_barcode (SampleBarcode), sample_type (SampleType),
        # subcore_sample (SubCoreSample), and env_measurements through survey
        queryset = FieldSample.objects.filter(sample_type__sample_type_code='ex').select_related(
            'survey_global_id',
            'field_sample_barcode',
            'sample_type',
            'subcore_sample'
