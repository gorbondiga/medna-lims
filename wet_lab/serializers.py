...
class MixsWaterSerializer(serializers.ModelSerializer):
    """
    Serializer for water FieldSample data in MIxS format
    Updated to work with FieldSample model directly
    """
    sample_id = serializers.UUIDField(source='sample_global_id', read_only=True)
    barcode = serializers.CharField(source='barcode_slug', read_only=True)
    
    class Meta:
        model = FieldSample
        fields = ['sample_id', 'barcode', 'project_name', 'lat_lon', 'depth',
                  ...
                  'nucl_acid_ext', 'geographic_location_latitude', 'geographic_location_longitude',
                  'geographic_location_country', 'temperature', 'ph', 'turbidity', 'conductivity', 'dissolved_oxygen']
    
    # Survey and Site fields
    project_name = serializers.ReadOnlyField(source='survey_global_id.mixs_project_name')
    lat_lon = serializers.ReadOnlyField(source='survey_global_id.mixs_lat_lon')
    geo_loc_name = serializers.ReadOnlyField(source='survey_global_id.site_id.mixs_geo_loc_name')
    env_broad_scale = serializers.ReadOnlyField(source='survey_global_id.site_id.mixs_env_broad_scale')
    env_local_scale = serializers.ReadOnlyField(source='survey_global_id.site_id.mixs_env_local_scale')
    env_medium = serializers.ReadOnlyField(source='survey_global_id.mixs_env_medium')
    
    # Geographic coordinates for ENA submission
    geographic_location_latitude = serializers.SerializerMethodField()
    geographic_location_longitude = serializers.SerializerMethodField()
    geographic_location_country = serializers.SerializerMethodField()
    
    # Collection date
    collection_date = serializers.SerializerMethodField()
    
    # Filter sample fields (water-specific)
    samp_collect_device = serializers.ReadOnlyField(source='filter_sample.mixs_samp_collect_device')
    samp_mat_process = serializers.ReadOnlyField(source='filter_sample.mixs_samp_mat_process')
    samp_size = serializers.ReadOnlyField(source='filter_sample.mixs_samp_size')
    
    # Sample type and method
    sample_type = serializers.CharField(source='sample_type.sample_type_label', read_only=True)
    sampling_method = serializers.CharField(read_only=True)
    
    # Extraction (if exists)
    nucl_acid_ext = serializers.SerializerMethodField()
    
    # Environmental measurements (from env_measure)
    depth = serializers.SerializerMethodField()
    temperature = serializers.SerializerMethodField()
    ph = serializers.SerializerMethodField()
    turbidity = serializers.SerializerMethodField()
    conductivity = serializers.SerializerMethodField()
    dissolved_oxygen = serializers.SerializerMethodField()
    
    def get_geographic_location_latitude(self, obj):
        """Extract latitude from lat_lon - ENA requires max 8 decimal places"""
        if hasattr(obj, 'survey_global_id') and obj.survey_global_id:
            geom = obj.survey_global_id.geom
            if geom:
                # Round to 8 decimal places to match ENA pattern requirement
                return f'{geom.y:.8f}'
        return None
    
    def get_geographic_location_longitude(self, obj):
        """Extract longitude from lat_lon - ENA requires max 8 decimal places"""
        if hasattr(obj, 'survey_global_id') and obj.survey_global_id:
            geom = obj.survey_global_id.geom
            if geom:
                # Round to 8 decimal places to match ENA pattern requirement
    ...
    def get_geographic_location_country(self, obj):
        """Get country from site"""
        if hasattr(obj, 'survey_global_id') and obj.survey_global_id:
            if hasattr(obj.survey_global_id, 'site_id') and obj.survey_global_id.site_id:
                # You may need to add a country field to FieldSite model
                return getattr(obj.survey_global_id.site_id, 'country', 'not provided')
        return 'not provided'
    
    def get_collection_date(self, obj):
        """Get collection date in ISO 8601 format (YYYY-MM-DD or YYYY-MM-DDTHH:MM:SSZ)"""
        if obj.sample_datetime:
            # Return ISO 8601 format date only (YYYY-MM-DD)
            return obj.sample_datetime.strftime('%Y-%m-%d')
        return 'not provided'
    
    def get_nucl_acid_ext(self, obj):
        """Get extraction SOP if extraction exists"""
        # Check if extraction relationship exists
        if hasattr(obj, 'extraction'):
            try:
                return obj.extraction.mixs_nucl_acid_ext
            except:
                pass
        return None
    
    def get_env_measurement(self, obj, measure_type):
        """Helper to get environmental measurement by type - returns just the value without units for ENA compliance"""
        if hasattr(obj, 'survey_global_id') and obj.survey_global_id:
            env_measurements = obj.survey_global_id.env_measurements.filter(
                env_measure_type__env_measure_type_code=measure_type
            ).first()
            if env_measurements:
                # Return just the value for ENA compliance
                return str(env_measurements.env_measure_value)
        return None
    
    def get_depth(self, obj):
        return self.get_env_measurement(obj, 'depth')
    
    def get_temperature(self, obj):
        return self.get_env_measurement(obj, 'water_temp')
    
    def get_ph(self, obj):
        return self.get_env_measurement(obj, 'ph')
    
    def get_turbidity(self, obj):
        return self.get_env_measurement(obj, 'water_turbidity')
    
    def get_conductivity(self, obj):
        return self.get_env_measurement(obj, 'conductivity')
    
    def get_dissolved_oxygen(self, obj):
        """Get dissolved oxygen and convert from mg/L to µmol/kg for ENA compliance"""
        value = self.get_env_measurement(obj, 'do')
        if value:
            try:
                # Convert mg/L to µmol/kg
                # Molecular weight of O2 = 32 g/mol
                # Assuming seawater density ~1.025 kg/L
                mg_per_l = float(value)
                umol_per_kg = (mg_per_l / 32) * 1000 / 1.025
                return f'{umol_per_kg:.2f}'
...
class MixsSedimentSerializer(serializers.ModelSerializer):
    """
    Serializer for sediment FieldSample data in MIxS format
    Updated to work with FieldSample model directly
    """
    sample_id = serializers.UUIDField(source='sample_global_id', read_only=True)
    barcode = serializers.CharField(source='barcode_slug', read_only=True)
    ...
    class Meta:
        ...
        fields = ['sample_id', 'barcode', 'project_name', 'lat_lon', 'depth',
                  ...
                  'sample_type', 'sampling_method', 'samp_collect_device', 'samp_mat_process', 'samp_size',
                  'nucl_acid_ext', 'geographic_location_latitude', 'geographic_location_longitude',
                  'geographic_location_country', 'temperature', 'ph']
    ...
    # SubCore sample fields (sediment-specific)
    samp_collect_device = serializers.ReadOnlyField(source='subcore_sample.mixs_samp_collect_device')
    samp_mat_process = serializers.ReadOnlyField(source='subcore_sample.mixs_samp_mat_process')
    samp_size = serializers.ReadOnlyField(source='subcore_sample.mixs_samp_size')
