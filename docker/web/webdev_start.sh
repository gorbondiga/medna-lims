...
if [ "x$DJANGO_DATABASE_LOADDATA" = 'xon' ]; then
  ...
  python ${APP_HOME}/manage.py loaddata ${FIXTURES_DIR}/wet_lab_amplificationmethod.json
  ...
  echo "${0}: [$(date -u)] ***Loading wet_lab_pcrreplicate"
  python ${APP_HOME}/manage.py loaddata ${FIXTURES_DIR}/wet_lab_pcrreplicate.json
  ...
  python ${APP_HOME}/manage.py loaddata ${FIXTURES_DIR}/freezer_inventory_freezerinventoryreturnmetadata.json
  # bioinformatics
...
python -m gunicorn --bind 0.0.0.0:8000 medna_metadata.wsgi \
