...
if [ "x$DJANGO_DATABASE_LOADDATA" = 'xon' ] && [ -d "${FIXTURES_DIR}" ]; then
  # Load fixtures
...
if [ "x$DJANGO_COLLECT_STATIC" = 'xon' ]; then
  ...
  python ${APP_HOME}/manage.py collectstatic --no-input --clear
fi

# Start server
echo "${0}: [$(date -u)] ***Starting server"
gunicorn --bind 0.0.0.0:8000 medna_metadata.wsgi --workers $CELERYD_NUM_NODES --log-level=info --log-config || exit 1
