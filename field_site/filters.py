...
class FieldSiteFilter(filters.FilterSet):
    ...
    watershed_type = filters.ChoiceFilter(choices=FieldSite._meta.get_field('watershed_type').choices,
                                          widget=CustomSelect2Multiple)

    class Meta:
        model = FieldSite
        fields = ['general_location_name', 
                  'watershed_type', 
                  'created_datetime']
...
class FieldSiteSerializerFilter(filters.FilterSet):
    ...
    class Meta:
        model = FieldSite
        fields = ['created_by', 'project', 'envo_biome_first',
