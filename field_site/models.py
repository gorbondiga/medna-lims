...
from utility.enumerations import WatershedTypes
...
class FieldSite(DateTimeUserMixin):
    # Django auto-creates an auto-incremental 'id' field as primary key (BigAutoField)
    # This is what ForeignKeys reference by default
    
    # Human-readable site identifier (e.g., US01, US02)
    # This is NOT the primary key, just a unique business identifier
    site_id = models.CharField('Site ID', unique=True, max_length=255)
    general_location_name = models.CharField('Location', null=True, 
                                             max_length=255)
    project = models.ManyToManyField('utility.Project', blank=True, 
                                     verbose_name='Affiliated Project(s)', 
                                     related_name='projects')
    purpose = models.CharField('Site Description/Purpose', max_length=255)
    ...
    # django srid defaults to 4326 (WGS84)
    country_code = models.CharField('Country code', max_length=6)
    geom = models.PointField('Latitude, Longitude (DD WGS84)', srid=4326)
    watershed_type = models.CharField('Watershed Type', 
                                      max_length=50, 
                                      choices=WatershedTypes.choices, 
                                      blank=True)
    #koppenCode = models.CharField()...
    ...
    def mixs_geo_loc_name(self):
        ...
        return '{country};{name}'.format(country=self.country_code, name=self.general_location_name)
    ...
    def __str__(self):
        return '{label}'.format(label=self.general_location_name)

    def save(self, *args, **kwargs):
        # site_id must be provided manually
